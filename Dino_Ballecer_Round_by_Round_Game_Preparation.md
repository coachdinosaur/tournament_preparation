...
Standalone operational playbook for **FM Dino Ballecer (2372, PHI)** covering all nine rounds, **22–27 June 2026** (times Asia/Manila, UTC+8). It gives deep round-by-round preparation for Rounds 1–2 and 4–9. Round 3 (FM Arlan Cabe, a PHI compatriot) is deliberately kept to a brief public-file card under the shared-information integrity gate — not a preparation target. This document is separate from the Tournament Preparation Manual and does not modify it.
...
|---|---|---|---|---|---|
| R1 · Mon Jun 22 | 16:00 · B5 | ● Black | IM Morris, James (AUS) | 2423 | **½–½, completed.** Caro-Kann Two Knights, Mindeno Exchange (`3...Bg4`); 46 moves |
| R2 · Tue Jun 23 | 10:00 · B1 | ● Black | IM Tan, Jun Ying (MAS) | 2404 | **1–0, completed.** Slav Defense, Quiet Variation (`4.e3 Bf5`); 34 moves |
| R3 · Tue Jun 23 | 16:00 · B5 | ○ White | FM Arlan Cabe (PHI) | 2298 | **NEXT.** Bb5 anti-Sicilian; public-file only (shared-coach gate) |
...
*Completed Mon Jun 22, 16:00, Board 5. Source: official Round 1 PGN + third improved seed report + DB.*

> **Result — ½–½ (46 moves).** Morris–Ballecer, Singapore, 22 June 2026. Dino scored a solid draw with Black against the higher-rated IM and begins the event on **0.5/1**.

### Post-game update — Round 1

- **Actual opening.** Dino chose the **Caro-Kann Two Knights, Mindeno Variation, Exchange Line (B11)**: `1.e4 c6 2.Nc3 d5 3.Nf3 Bg4 4.h3 Bxf3 5.Qxf3 Nf6`, using the playbook's low-priority Caro-Kann alternative rather than the recommended Taimanov/Kan.
- **Game shape.** Black accepted a sharp kingside fight with `...h5`, `...f5`, and `...g5`. All four rooks came off in the forcing sequence `26...Ng5 27.Bxg5 Qxg5 28.Rxf7 Rxf7 29.Rxf7 Kxf7`; the game then settled into queen-and-minor-piece play.
- **Finish.** After `37...Nxe6 38.Qxe6+`, Morris forced a repeated checking sequence. The draw was agreed after `46.Qh5+`.
- **Prep implication.** The result upgrades this exact `3...Bg4` Caro-Kann line from an untested fallback to a **live practical backup**, but one draw is not enough to replace the event's narrow Taimanov/Kan default. Add the Morris game to the model file and review the critical decisions at `12...b4`, `14...f5`, `16...fxe4`, and `26...Ng5` after the Round 2 recovery window.
- **Immediate recovery rule.** Do not spend the short turnaround deeply analysing Round 1 tonight. Save the full post-mortem; use the remaining prep window only for Tan's `1.Nf3 / 1.d4` move orders and the `...dxc4` plan.

### Original pre-game dossier
...
**Critical position (after the Taimanov reference line, White to move 7):** `r1b1kbnr/1pqp1ppp/p1n1p3/8/3NP3/2N1B3/PPP2PPP/R2QKB1R w KQkq - 0 7`. *Morris-specific:* he plays quiet Sicilians — most likely `Be2 + O-O + f3` rather than English Attack, so plan `...Bd6`, `...Nge7`, and `...b5` at leisure. Plans/breaks: **Systems Library — Taimanov**.
...
**4 — Caro-Kann: low-priority alternative.** `(C)`/`(I)` Morris scores only **33.3%** in the Caro-Kann Exchange as White. Before Round 1 Dino had no practical Caro-Kann reps; the completed Morris game is now his first live reference, though it reached the Two Knights rather than this Exchange line: `1.e4 c6 2.d4 d5 3.exd5 cxd5 4.Bd3 Nc6`.
...
- ☐ Taimanov vs Kan choice locked
- ☐ Bb5 systems confirmed irrelevant after `2...e6`
- ☐ Dragon explicitly off
- ☐ Neo-Grünfeld vs `1.Nf3/1.c4` recalled
- ☐ one active-rook hold rehearsed

### In-Game Card — R1 ● Black vs Morris (IM, 2423)

**Status.** Completed — draw, ½–½ in 46 moves.

**Opening choice.** Narrow Taimanov via `2...e6`. Triggers: `2.c3 → Alapin · 2.Nc3 + 3.g3 → Closed · 2.Nf3 + 3.d4 → Taimanov/Kan · 2.Nf3 + 3.d3/g3 → KIA · 2.Nf3 + 3.b3 → Westerinen`.

**Three middlegame plans (Black).** (1) `...b5–b4` to displace `Nc3`; (2) `...d5` only after `...a6/...b5/...Bb7/...Rc8` are in place; (3) keep pieces on toward the endgame.


**Three middlegame plans (Black).** (1) `...b5–b4` to displace `Nc3`; (2) `...d5` only after `...a6/...b5/...Bb7/...Rc8` are in place; (3) keep pieces on toward the endgame.

**Avoid.** Dragon move-order; `2...Nc6` (invites Rossolimo).

**Time budget (90+30, inc 30).** Opening to move 12 ≤ 20m · moves 13–25 ≤ 50m · moves 26–40 ≤ 50m · reserve 5m for time-trouble.

**Win condition.** Quiet middlegame → his endgame (ACPL 36.2) is the field's loosest.

## Round 2 — ● Black vs IM Tan, Jun Ying (2404)

*Completed Tue Jun 23, 10:00, Board 1. Source: official Round 2 PGN + fourth improved seed report + DB.*

> **Result — 0–1 (34 moves).** Tan–Ballecer, Singapore, 23 June 2026. Dino lost with Black and is on **0.5/2** after two rounds.

### Post-game update — Round 2

- **Actual opening.** Dino chose the **Slav Defense, Quiet Variation (D12)**: `1.d4 d5 2.c4 c6 3.Nf3 Nf6 4.e3 Bf5 5.Nc3 e6 6.Nh4 Be4 7.f3 Bg6`, deviating from the recommended Catalan/QGD system (`...Nf6 / ...e6 / ...d5` with `...dxc4`).
- **Game shape.** Tan went for an early `Nh4` to target Black's light-squared bishop, obtaining the bishop pair. After castling long, White established control over the center and queenside. Dino tried to open the center with `16...e5`, but Tan's piece activity and kingside pawn storm were superior. Dino was forced to sacrifice an exchange with `25...Rxc5 26.dxc5` to hold on.
- **Finish.** After `30.Qb5 Qxb5 31.Bxb5 Rb8 32.Ba6 Rb6 33.Rc8+ Kh7 34.Bd3`, Dino resigned due to the material deficit and White's active pieces.
- **Prep implication.** Slav Defense was played instead of the primary Catalan/QGD prep. The early `Nh4` bishop hunt proved highly effective for White. Review the Slav lines if they are to be kept as a backup, but default to the primary QGD/Catalan system for future `1.d4/1.Nf3` pairings.
...
**Critical position (after the `1.Nf3` Anti-Catalan reference line, White to move 7):** `rnbq1rk1/ppp1bppp/4pn2/8/2pP4/5NP1/PP2PPBP/RNBQ1RK1 w - - 0 7`. *Tan-specific:* he'll try `Qc2 + a4` (the structural squeeze to recover c4) — meet with `...a6 + ...b5 + ...Bb7`. Plans/breaks: **Systems Library — Anti-Catalan with `...dxc4`**.
...
**Match-day checklist (and double-round opener):**

- ☐ one move-order memorized for `1.Nf3`/`1.d4`/`1.c4`
- ☐ `...d5–d4` surprise checked or shelved
- ☐ `1.e4` → Taimanov, not Dragon
- ☐ recovery plan for R3 same evening

### In-Game Card — R2 ● Black vs Tan (IM, 2404)

**Status.** Completed — loss, 0–1 in 34 moves.

**Opening choice.** One `...Nf6/...e6/...d5` Catalan/QGD structure for `1.Nf3 / 1.d4 / 1.c4`; `...dxc4` to unbalance. Vs rare `1.e4` → Taimanov (not Dragon).

**Three middlegame plans (Black).** (1) `...c5` early to free the position; (2) hold the c4-pawn with `...a6 + ...b5 + ...Bb7`; (3) press into a rook ending — his record is 16 draw-defense failures.

**Avoid.** Symmetric KID lines (he is strong); the early `...d5–d4` Réti surprise unless re-checked just before the game.

**Time budget.** Create the first real problem by move 10–12 — his rapid/blitz is far below classical; make him think.
...
> **Integrity gate.** Cabe is a PHI compatriot. This card is a **public-file audit only** — built from his published game record, with **no use of shared-coaching information**. Treat every read as a tendency from public games. **By the player's own choice, no deep preparation is built for this compatriot pairing** — the brief notes below are public-record context only, not a worked-up game plan.
...
### In-Game Card — R3 ○ White vs Cabe (FM, 2298)

**Integrity gate.** No deep prep used. Play a normal `1.e4` game on general understanding — no scouting edge is implied. *Recovery focus:* this evening game needs no opening prep, then early sleep for the R4 double-round morning.
