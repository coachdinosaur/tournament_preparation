...
- **134 games on file** = **99 over-the-board PGN games** + **35 Lichess online metadata rows**.
...
- **The single most important read in this file: separate his classical play from his online play.**
  ...
  that is dragged down by 39 online Titled Tuesday rapid games. **Restricted to his 60 classical
...
- **Best classical target when Dino has White:** the **Advance Caro-Kann against his Black**. His
  ...
  This is a strong, fresh, result-based target — but a workload warning, because the engine says he
  defends it cleanly (ACPL 15.0, 0 blunders) and Dino has zero Advance Caro reps in his own file.
- **Lowest-friction White fit:** if Chan answers 1.e4 with the French, use Dino's proven **King's
...
- **One-sentence game plan:** *With White, meet his French with the proven KIA and prepare the
  ...
  squeeze and steer toward his draw-heavy fianchetto setups, while keeping the game classical-rich
  rather than letting it become the kind of speed shuffle where his blunder rate spikes.*
...
Advance Caro-Kann is the headline leak; the Petrov and Classical Caro-Kann are secondary result
...
| Petrov's Defense: Nimzowitsch Attack as Black | C42 | 2 | 0.0% | Candidate White target on a small sample; needs a 1.e4 e5 plan |
...
> blitz losses, so treat it as speed noise, not a classical White-side weakness.

### Repertoire-fit check

- **White plan A - KIA vs the French (lowest friction).** Dino's own King's Indian Attack is a
  tested 91.7% weapon over 6 games. Chan plays the French; his most-played French line is 50%. This
  is the highest-confidence, zero-new-prep path.
...
*Result score and engine accuracy are separate evidence types. The table below is the **classical-only**
...
|---|---|---:|---:|---:|---:|---:|---|
| Advance Caro-Kann | Black | 5 | **20.0%** | 162 | 15.0 | 0/3 | Top result target; he defends cleanly, so out-prep not out-blunder |
...
| Reversed-Sicilian / Flank | Black | 2 | 25.0% | 72 | 24.7 | 1/4 | Small-sample volatility; pointer only |
...
- **Anti-Sicilian footnote:** the dossier shows him at 80% in the Anti-Sicilian family (5 games), but
  those are Lichess metadata rows with no move list, so there is no engine confirmation - treat it as
...
weakness in the raw numbers is largely an online-speed artifact. The realistic chance is to keep the
...
1. **Keep the game classical-rich; do not let it become a speed shuffle.** His error rate roughly
...
- **Coverage:** 134 total games, 99 OTB PGNs, 35 Lichess metadata rows, 99/99 OTB analysed at depth
  12, 0 pending.
...
- **Evidence discipline applied:** every major claim carries a count, score, ACPL, or concrete game
  sample; result-score targets and engine-accuracy targets are reconciled explicitly (notably the
...
- **Known limitations:** depth 12 is a screening depth, not final engine truth; several targets rest
  on 2-5 game samples; Lichess metadata rows are opening/result only and are excluded from all engine
