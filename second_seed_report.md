...
- **Engine claims below are from 101/101 OTB PGN games analysed by Stockfish 18 at depth 12.**
  There are **0 pending engine games**. Unlike the first seed, there is no Lichess-only metadata
  layer here.
...
- **Best candidate target as White:** stress his Sicilian setup. The dossier flags his **Najdorf
...
- **Do not flatten the evidence.** Shyaam still scores **4W 0D 1L (80.0%)** as Black in the
  Anti-Sicilian family, so this is an engine-pressure target, not a result-score weakness.
...
**As Black - 52 games, 58.7%.** His Black file is more targetable. The main candidate is the
...
| Catalan / Reti family as Black | A/E systems | 4 | 50.0% | Weak result flag, but engine-clean; not a standalone target |
...
*Engine fingerprint over the core structure families at depth 12. Result score and engine accuracy
are separate evidence types; where they disagree, the report says so.*

| Structure family | Games (result) | Result % | Moves | ACPL | Serious err | Read |
|---|---:|---:|---:|---:|---:|---|
| Anti-Sicilian Counter-Package | 12 | 66.7% | 425 | **14.6** | **7** | Engine target, but not a result weakness |
...
- **Coverage:** 101 total games, 101 OTB PGNs, 0 Lichess metadata rows, 101/101 analysed at depth 12,
  0 pending.
- **Evidence discipline applied:** every major claim has a count, score, ACPL, or concrete game
  sample; Lichess metadata is separated from engine analysis; result-score targets and
  engine-accuracy targets are reconciled explicitly.
