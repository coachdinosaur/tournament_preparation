...
- **102 games on file** = **102 over-the-board PGN games** + **0 Lichess online metadata rows**.
- **Engine claims below are from 102/102 OTB PGN games analysed by Stockfish 18 at depth 12.** There
  are **0 pending engine games**. (The shipped dossier had a stale 2-of-102 cache reading a nonsense
...
stable, trusted lines a now-coaching player is most likely to wheel out, but it also means **expect
OTB rust** in long-game calculation and clock handling.

- **Separate his classical play from his speed play.** Across all 102 games his profile looks volatile
  ...
  21.9, endgame ACPL 30.0, 24 blunders.** Do not build the plan around a speed-inflated collapse.
...
- **Cleanest result target (Dino with White):** his **Caro-Kann Advance, Short Variation** is **0W 0D
  ...
  appears if Dino plays the **Advance (3.e5)**, and Dino's audit flags **zero Advance Caro-Kann reps**.
...
- **The Advance Short target is a project, not a surprise.** The 0/2 result is real but tiny, and
  Dino has **zero Advance Caro-Kann reps** (a flagged gap in his audit). Only choose the Advance if
...
*Classical-only structure split by Thejkumar's color (result **and** engine accuracy, which are
separate evidence types).*

| Structure family | His color | Games | Result % | ACPL | B/M | Read |
|---|---|---:|---:|---:|---:|---|
| Catalan / Réti Squeeze | White | 7 | **42.9%** | 19.1 | 1/10 | Soft result (2-2-3) but clean engine play — out-result, not out-blunder |
...
| IQP / Panov Attack | White | 3 | **33.3%** | 13.6 | 1/1 | Soft result (0-2-1) on a tiny sample; clean engine play |

**Practical structure verdicts:**

- **Soft-by-result, clean-by-engine.** His two weakest classical White families (Catalan/Réti 42.9%,
  IQP/Panov 33.3%) show low ACPL, so the losses are about being **out-played/ground down**, not
  blundering. For Dino that means these are *playable, double-edged* targets that reward genuine
...
The dossier's raw "endgame ACPL 281.0 / 7 blunders" was a 2-game cache artifact and is discarded. On
...
- **Coverage:** 102 total games, 102 OTB PGNs, 0 Lichess rows, **102/102 analysed at depth 12**, 0
  pending. The shipped dossier's 2-of-102 cache was discarded and the full set re-analysed.
...
- **Known limitations:** depth 12 is a screening depth, not final engine truth; classical family
  samples are 2–7 games each; result-score targets and engine-accuracy targets are reconciled
  explicitly (his soft White families are clean by ACPL, i.e. out-played not out-blundered).
