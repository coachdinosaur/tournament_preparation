...
- **Best candidate target as White:** steer toward Catalan/Reti squeeze structures against his
...
- **Do not overfit the engine table.** The Fianchetto KID / Grunfeld complex is engine-volatile
  (**ACPL 29.0**) but Morris scores **80.0%** there as Black. Treat it as a study zone, not an
  automatic target.
...
Nimzo-Larsen move orders. Dino should expect flexibility rather than one fixed tabiya.
...
| Caro-Kann Defense: Exchange Variation as White | B13 | 3 | 33.3% | Candidate Black target only if Dino can play the Caro-Kann confidently |
...
- **Steer toward with White:** Catalan/Reti pressure if Dino can prepare the move order. This has
  both a result-score signal and an engine-volatility signal when Morris has Black.
...
- **Coverage:** 100 total games, 100 OTB PGNs, 0 Lichess metadata rows, 100/100 analysed at depth
  12, 0 pending.
